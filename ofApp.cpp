...
void ofApp::draw() {
	...
	for (int r = 0; r < 5; r++) {
		...
		auto radius = 350 - r * 20;
		auto noise_param = glm::vec3(ofRandom(1000), ofRandom(1000), ofRandom(1000));

		auto angle_x = ofMap(ofNoise(noise_param.x, ofGetFrameNum() * 0.0001), 0, 1, -PI * 1.5, PI * 1.5);
		auto angle_y = ofMap(ofNoise(noise_param.y, ofGetFrameNum() * 0.0001), 0, 1, -PI * 1.5, PI * 1.5);
		auto angle_z = ofMap(ofNoise(noise_param.z, ofGetFrameNum() * 0.0001), 0, 1, -PI * 1.5, PI * 1.5);
